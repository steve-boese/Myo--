...
$delim = (strpos(dirname(__FILE__),"\\") > 0) ? "\\" : "/";
$path = explode($delim,dirname(__FILE__));array_pop($path);array_pop($path);$path = implode($delim,$path);
define( 'ABSPATH', $path . $delim );

include (ABSPATH . 'admin/config.php');

if (!isset($_SESSION[SITE_PORT]['user_id'])) exit;

if ($_SESSION[SITE_PORT]['user_type'] > 2) {
  $modules = explode(',',$_SESSION[SITE_PORT]['user_modules']);
  if (!in_array(23,$modules))
    exit;
}
 ...
 */
$kfm_userfiles_address = ABSPATH . '/site/image';
...
$kfm_userfiles_output =  '/site/image/';
